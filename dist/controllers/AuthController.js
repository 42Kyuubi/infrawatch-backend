...
class AuthController {
    async signUp(req, res) {
        const parsed = SignUpSchema_1.signUpSchema.safeParse(req.body);
        if (!parsed.success) {
    ...
    async signIn(req, res) {
        const parsed = SignInSchema_1.signInSchema.safeParse(req.body);
        if (!parsed.success) {
            const errors = parsed.error.format();
            return res.status(400).json({ errors });
        }
    ...
    async refresh(req, res) {
        try {
            ...
            const result = await AuthService_1.default.refresh(refresh_token);
            return res.status(200).json(result);
        }
        catch (err) {
            return res.status(401).json({ error: err.message });
