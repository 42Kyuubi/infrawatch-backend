...
async function createTicket(ticketData) {
    ...
    try {
        ...
        const sessionRes = await axios_1.default.get(apiUrl + AUTH_SESSION_ENDPOINT, {
            headers: headersAuth,
        });
        ...
        const createRes = await axios_1.default.post(apiUrl + TICKET_ENDPOINT, payload, {
            headers,
        });
        console.log("Ticket Created:", createRes.data);
        const ticketsRes = await axios_1.default.get(apiUrl + TICKET_ENDPOINT, { headers });
        console.log("Tickets:", JSON.stringify(ticketsRes.data, null, 2));
        console.log(`### Killing actual session (${sessionToken}) ###`);
        const killRes = await axios_1.default.get(apiUrl + KILL_SESSION_ENDPOINT, {
            headers,
        });
